...
const ALL_MODELS = [
  ...
  { id: 'GEMINI_FLASH_LITE_LATEST', name: 'Gemini Flash Lite (Latest)', desc: 'Optimized Speed & Cost', icon: Zap, color: 'text-[var(--text-secondary)]' },
...
export const Upload = forwardRef<HTMLDivElement, UploadProps>(({ onExtractionComplete }, ref) => {
  ...
  return (
    <div className="space-y-5" ref={ref}>
      {/* Controls Bar */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-4 bg-[var(--bg-main)] rounded-xl border border-[var(--divider)]">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center">
            <Brain className="w-4 h-4 text-primary" />
          </div>
          <div className="flex bg-white border border-[var(--border-input)] p-0.5 rounded-lg shadow-sm">
            <button
              className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${
                uploadMode === 'file' ? 'bg-slate-900 text-white shadow-sm' : 'text-[var(--text-secondary)] hover:text-slate-800'
              }`}
            ...
            <button
              className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${
                uploadMode === 'text' ? 'bg-slate-900 text-white shadow-sm' : 'text-[var(--text-secondary)] hover:text-slate-800'
              }`}
        ...
        <Select value={selectedModel} onValueChange={setSelectedModel}>
          <SelectTrigger className="w-full md:w-[220px] h-9 rounded-lg bg-white border-[var(--border-input)] font-bold text-[var(--text-primary)] text-xs shadow-sm focus:ring-primary/10">
          ...
          <SelectContent className="rounded-xl border-none shadow-2xl p-1.5">
            <SelectItem value="smart" className="rounded-lg py-2.5 focus:bg-primary/5 cursor-pointer">
                <div className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center text-primary bg-[var(--bg-main)] border border-[var(--divider)]">
                    <Brain className="w-3.5 h-3.5" />
                  </div>
                  <div className="flex flex-col">
                    <span className="font-bold text-[var(--text-primary)] text-xs">Auto-Detect AI</span>
                    <span className="text-[10px] text-[var(--text-muted)] font-medium">Global Settings Default</span>
                  </div>
                </div>
            </SelectItem>
            {availableModels.map((model) => (
              <SelectItem key={model.id} value={model.id} className="rounded-lg py-2.5 focus:bg-primary/5 cursor-pointer">
                <div className="flex items-center gap-3">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${model.color} bg-[var(--bg-main)] border border-[var(--divider)]`}>
                    <model.icon className="w-3.5 h-3.5" />
                  </div>
                  <div className="flex flex-col">
                    <span className="font-bold text-[var(--text-primary)] text-xs">{model.name}</span>
                    <span className="text-[10px] text-[var(--text-muted)] font-medium">{model.desc}</span>
                  </div>
                </div>
              </SelectItem>
            ))}
      ...
      {uploadMode === 'file' ? (
        ...
        <div className="relative group">
          <textarea
            className="w-full h-64 p-6 bg-white border-2 border-[var(--divider)] rounded-3xl outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/5 transition-all text-[var(--text-primary)] leading-relaxed shadow-sm"
      ...
      {loading && (
        <div className="space-y-4 bg-white p-6 rounded-xl border border-[var(--divider)] shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[var(--bg-main)] p-3 rounded-xl border border-[var(--divider)]">
              <div className="flex items-center gap-1.5 mb-1">
                <Clock className="w-3.5 h-3.5 text-[var(--text-muted)]" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)]">Est. Time</span>
              </div>
            ...
            <div className="bg-[var(--bg-main)] p-3 rounded-xl border border-[var(--divider)]">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)]">Engine</span>
              </div>
              <p className="text-sm font-black text-[var(--text-primary)]">AI Active</p>
            </div>
          </div>
        </div>
      )}
