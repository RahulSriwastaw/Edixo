...
import { BookOpen, Plus, Search, Loader2, Edit2, Trash2, Globe, Eye, Filter, X } from 'lucide-react';
...
export default function MyQuestionsPage() {
    ...
    const stripHtml = (html?: string, truncate = false) => {
        if (!html) return '';
    ...
    return (
        <div className="animate-fade-in" style={{ maxWidth: 1200, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <div>
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>
                        {total} questions in your personal bank
                    </p>
                </div>
                <Link href="/dashboard/create-question">
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 20 }}>
                ...
                <select className="db-input" style={{ width: 140 }} value={sourceFilter}
                ...
                <select className="db-input" style={{ width: 140 }} value={visibilityFilter}
            ...
            {loading ? (
                ...
                <div className="db-card" style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--text-muted)' }}>
                    ...
                    <Link href="/dashboard/create-question">
                        <button className="btn btn-secondary btn-sm" style={{ marginTop: 16 }}>Create Question</button>
                    </Link>
                </div>
            ) : (
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ fontSize: 13, color: 'var(--text-primary)', lineHeight: 1.5, marginBottom: 4 }}>
                                        {stripHtml(q.textEn, true)}
                                    </div>
                                    {q.sourcePackName && (
                                        <div style={{ fontSize: 11, color: 'var(--badge-info-text)', marginTop: 2 }}>
                                            From pack: {q.sourcePackName}
                                        </div>
                                    )}
                                ...
                                <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                                    ...
                                    <Link href={`/dashboard/edit-question/${q.id}`}>
                                    ...
                                    <button className="btn btn-ghost btn-sm btn-icon" style={{ color: 'var(--badge-error-text)' }}
                                        onClick={() => { setQuestionToDelete(q); setShowDeleteDialog(true); }} title="Delete">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in" onClick={() => setPreviewQ(null)}>
                    <div className="db-card" style={{ width: '100%', maxWidth: 560, padding: 20, maxHeight: '80vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                            {getSourceBadge(previewQ.source)}
                            <span className="pill pill-muted">{previewQ.type?.replace('_', ' ') || 'MCQ'}</span>
                            <span className="pill pill-muted">{previewQ.difficulty}</span>
                        </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
                                {previewQ.options.map((opt: any, i: number) => (
                                    <div key={i} style={{
                                        ...
                                        background: opt.isCorrect ? 'var(--badge-success-bg)' : 'var(--bg-input)',
                                        border: `1px solid ${opt.isCorrect ? 'var(--badge-success-text)' : 'var(--border-input)'}`,
                                        color: opt.isCorrect ? 'var(--badge-success-text)' : 'var(--text-primary)',
                                        fontSize: 13
                                    }}>
                                        {String.fromCharCode(65 + i)}. {opt.textEn}
                                        {opt.isCorrect && <span style={{ marginLeft: 8, fontWeight: 600 }}>✓ Correct</span>}
                                    </div>
                                ))}
                            </div>
                        )}
                        {previewQ.explanationEn && (
                            <div style={{ padding: 10, background: 'var(--badge-info-bg)', borderRadius: 'var(--radius-btn)', fontSize: 12, color: 'var(--badge-info-text)' }}>
                                <strong>Explanation:</strong> {stripHtml(previewQ.explanationEn)}
                            </div>
                        )}
                    </div>
                </div>
            ...
            {showDeleteDialog && questionToDelete && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
                    <div className="db-card" style={{ width: '100%', maxWidth: 420, padding: 20 }}>
                        <h2 className="page-title" style={{ marginBottom: 8, color: 'var(--badge-error-text)' }}>
                            <Trash2 size={18} style={{ display: 'inline', marginRight: 6 }} />
                            Delete Question
                        </h2>
