    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (d.success) {
                ...
                alert(d.message || 'Failed to create set');
            }
        } catch {
    ...
    return (
        <div className="animate-fade-in" style={{ maxWidth: 1200, margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <button className="btn btn-primary" onClick={() => setShowCreate(true)}><Plus size={14} /> New Set</button>
            </div>

            <div style={{ position: 'relative', marginBottom: 20 }}>
                <Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
                <input className="db-input" style={{ paddingLeft: 32 }} placeholder="Search sets..." value={search}
                    ...
                    {sets.map((set: any) => (
                        <div key={set.id} className="db-card" style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                                <button className="btn btn-secondary btn-sm" style={{ flex: 1 }} onClick={() => handleViewSet(set.id)}>
                                    <Eye size={13} /> View
                                </button>
                                <button className="btn btn-ghost btn-sm btn-icon" style={{ color: 'var(--badge-error-text)' }}
                                    onClick={() => handleDelete(set.id)}><Trash2 size={14} /></button>
                            </div>
                        </div>
                    ))}
                </div>
                    ...
                    <div className="db-card" style={{ width: '100%', maxWidth: 400, padding: 24 }}>
                        ...
                        <form onSubmit={handleCreate}>
                            <div style={{ marginBottom: 12 }}>
                                <label className="section-header" style={{ display: 'block', marginBottom: 6 }}>Set Name</label>
                                <input className="db-input" required placeholder="e.g., Physics Revision" value={newSetName}
                                    onChange={e => setNewSetName(e.target.value)} />
                            </div>
                            <div style={{ marginBottom: 16 }}>
                                <label className="section-header" style={{ display: 'block', marginBottom: 6 }}>Description</label>
                                <textarea className="db-input" style={{ minHeight: 60 }} placeholder="Optional description"
                                    value={newSetDesc} onChange={e => setNewSetDesc(e.target.value)} />
                            </div>
            ...
            {viewSet && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setViewSet(null)}>
                    <div className="db-card" style={{ width: '100%', maxWidth: 640, padding: 20, maxHeight: '80vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
                        ...
                        <p className="card-meta" style={{ marginBottom: 12 }}>{viewSet.description || 'No description'} • {viewSet.questionCount} questions</p>
                        {viewSet.questions && viewSet.questions.length > 0 ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {viewSet.questions.map((q: any, i: number) => (
                                    <div key={q.uqId} className="db-card" style={{ padding: 12 }}>
                                        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>#{i + 1}</div>
                                        <div style={{ fontSize: 13, color: 'var(--text-primary)' }} dangerouslySetInnerHTML={{ __html: q.textEn?.slice(0, 200) }} />
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <p style={{ color: 'var(--text-muted)', textAlign: 'center', padding: 20 }}>This set is empty.</p>
                        )}
                    </div>
                </div>
            )}
