...
export default function EditQuestionPage() {
        ...
        const fetchQuestion = async () => {
            ...
            try {
                ...
                if (d.success) {
                    ...
                    setSourceInfo({
                        source: q.source,
                        sourcePackName: q.sourcePackName,
                        sourcePackId: q.sourcePackId,
                    });
                }
            } catch (e) {
                console.error(e);
    ...
    if (loading) {
        return <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text-muted)' }}>Loading...</div>;
    }
    ...
    return (
        <div className="animate-fade-in" style={{ maxWidth: 800, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                ...
                <div>
                    <h1 className="page-title" style={{ marginBottom: 4 }}>Edit Question</h1>
                    <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>Update your question</p>
                </div>
            </div>
            ...
            {sourceInfo?.source === 'marketplace' && (
                <div className="db-card" style={{ marginBottom: 16, background: 'var(--badge-info-bg)', border: '1px solid rgba(33,150,243,0.2)' }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                        ...
                        <div>
                            <p style={{ fontSize: 13, color: 'var(--badge-info-text)', fontWeight: 500, marginBottom: 2 }}>
                                    ...
                                    <button
                                        onClick={() => {
                                            if (form.type === 'MCQ_SINGLE') {
                                                setOptions(options.map((o: any, idx: number) => ({ ...o, isCorrect: idx === i })));
                                            } else {
                                                setOptions(options.map((o: any, idx: number) => idx === i ? { ...o, isCorrect: !o.isCorrect } : o));
                                            }
                                        }}
                                        style={{
