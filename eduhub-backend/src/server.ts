...
async function generateUniqueSixDigitSetId(
  seed: string,
  excludeSetPrimaryId?: string,
) {
...
async function ensureCompatibilitySetsFromQuestions() {
  const sourceRows = await prisma.$queryRawUnsafe<
    Array<{ source: string; total: number | string }>
  >(`
        SELECT
  ...
  for (const row of sourceRows) {
    ...
    await prisma.$executeRawUnsafe(
      `DELETE FROM question_set_items WHERE set_id = ${sqlQuote(setPrimaryId)}`,
    );
...
async function ensureQuestionBankCompatibilityData() {
  ...
  const questionCountRows = await prisma.$queryRawUnsafe<
...
const staticOrigins = env.ALLOWED_ORIGINS.split(",").map((o) =>
  o.trim().replace(/\/$/, ""),
);
    ...
    origin: async (origin, callback) => {
      ...
      logger.warn(
        "CORS blocked for origin: " +
          origin +
          ". Add it to ALLOWED_ORIGINS to allow access.",
      );
...
async function startServer() {
  try {
    try {
      ...
      logger.warn(
        "Redis not available - running without cache/queues (dev mode only)",
      );
    }
    ...
    try {
      ...
      (async () => {
        try {
          ...
          logger.error(
            "Non-critical background compatibility check failed:",
            compatErr,
          );
        }
      })();
      ...
      logger.warn(
        "Database connection failed - server will start without initial compatibility checks (dev mode).",
      );
    }
