...
export const saveDraftQuestions = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
      ...
      while (retryCount <= MAX_RETRIES && !success) {
        try {
          await prisma.$transaction(
            ...
            { timeout: 20000 },
          ); // 20s timeout per chunk transaction
          ...
          if (retryCount > MAX_RETRIES) {
            logger.error(
              `Chunk ${chunkNumber} failed after ${MAX_RETRIES} retries. Error: ${err.message}`,
            );
            failedChunks++;
          } else {
            logger.warn(
              `Chunk ${chunkNumber} failed (Attempt ${retryCount}/${MAX_RETRIES + 1}), retrying... Error: ${err.message}`,
            );
            // Wait for a short duration before retrying (exponential backoff)
            await new Promise((resolve) =>
              setTimeout(resolve, retryCount * 500),
            );
          }
        }
      }
...
export const generateBlogDraftFromUrl = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    const schema = z.object({
      ...
      targetPlatform: z.enum(["mokebook", "public_website", "both"]).optional(),
      tone: z.string().max(200).optional(),
      extraInstructions: z.string().max(2000).optional(),
    });

    const body = schema.parse(req.body);
    const draft = await AIService.generateBlogDraftFromUrl(body);
    ...
    logger.error("AI Blog Draft Error:", err);
    next(err);
  }
};
...
export const getAISettings = async (
  ...
  try {
    let settings = await prisma.ai_settings.findUnique({
      where: { id: "singleton" },
    });
