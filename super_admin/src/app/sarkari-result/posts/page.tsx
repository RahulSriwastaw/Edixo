...
export default function PostsListPage() {
  ...
  useEffect(() => {
    ...
    setPostType(pt);
    fetch(`/api/sarkari/posts?postType=${pt}&page=1&limit=50`)
      .then((r) => r.json())
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/sarkari/posts/${id}`, { method: "DELETE" });
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[var(--text-secondary)] text-sm mt-1">
                    Manage jobs, results, admit cards, and more
                  </p>
                </div>
              </div>
              <Link href="/sarkari-result/new">
                        ...
                        {posts
                          ...
                          .map((p) => (
                            ...
                            >
                              <td className="p-3">
                                <Link
                                  href={`/sarkari-result/posts/${p.id}`}
                                  className="font-medium text-[var(--text-primary)] hover:text-[#2563EB]"
                                >
                                  {p.title}
                                </Link>
                              ...
                              <td className="p-3">
                                <span
                                  className={`text-xs px-2 py-1 rounded-full ${p.status === "published" ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-600"}`}
                                >
                                  {p.status}
                                </span>
                              </td>
                              ...
                              <td className="p-3">
                                <div className="flex items-center justify-end gap-2">
                                  <Link href={`/sarkari-result/posts/${p.id}`}>
                                    <Button variant="outline" size="sm">
                                      <Eye className="w-3 h-3" />
                                    </Button>
                                  </Link>
                                  <Link href={`/sarkari-result/new?id=${p.id}`}>
                                    <Button variant="outline" size="sm">
